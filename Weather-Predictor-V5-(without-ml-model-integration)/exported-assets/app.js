...
class WeatherPredictionApp {
    ...
    async fetchWeatherData() {
        ...
        this.weatherData = this.generateSimulatedWeatherData(location.lat, location.lng);
    }
    ...
    async fetchHealthData() {
        ...
        this.healthData = this.generateSimulatedHealthData(location.lat, location.lng);
    }
    ...
    calculateSuitabilityScore(weatherAnalysis, healthAnalysis, probabilities) {
        ...
        // Weather score (0-100)
        const weatherScore = weatherAnalysis.overallProbability;
        
        // Health score (0-100, inverted so lower AQI = higher score)
        const healthScore = Math.max(0, 100 - (healthAnalysis.aqi.value * 100 / 300));
        
        // Risk score (0-100, inverted so lower risk = higher score)
        const avgRisk = (probabilities.extremeHeat + probabilities.extremeCold + probabilities.highWind + probabilities.uncomfortable) / 4;
        const riskScore = Math.max(0, 100 - avgRisk);
        
        // Calculate weighted final score
        const finalScore = Math.round(
            (weatherScore * (weights.temp + weights.wind + weights.humidity + weights.precipitation)) +
            (healthScore * weights.air_quality) +
            (riskScore * 0.1)
        );
    ...
    prepareChartData(historical) {
        return {
            dates: historical.slice(-10).map(day => day.date.toLocaleDateString()),
            temperatures: historical.slice(-10).map(day => Math.round(day.temperature * 10) / 10),
            precipitation: historical.slice(-10).map(day => Math.round(day.precipitation * 10) / 10),
        };
    }
